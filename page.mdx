...
This notebook describes a spiking neural network (SNN) simulation that performs digit classification on the MNIST dataset. Using only NumPy for implementation, our model achieves over 90% classification accuracy on the MNIST test set. The network uses leaky integrate-and-fire (LIF) neurons and is trained using surrogate gradient descent. The implementation includes time-resolved Poisson encoding of images, a two-layer feedforward architecture, and logging of membrane potentials and spike activity throughout simulation and training.
